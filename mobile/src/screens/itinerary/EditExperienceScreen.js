...
const EditExperienceScreen = ({ navigation, route }) => {
    ...
    getItineraryById(id)
      .then((data) => {
        ...
        const dest = {
          ...
          coordinates: { lat: data.location?.lat ?? 0, lon: data.location?.lon ?? 0 },
        };
        setDestination(dest);
  ...
  const runGenerate = async () => {
    ...
    try {
      ...
      const generated = (data.places ?? []).map((p, i) => ({
        ...
        category: p.category ?? 'other', dayNumber: p.dayNumber ?? 1,
  ...
  const addStep = () => {
    ...
    const fresh = { _key: `new-${Date.now()}`, name: '', description: '', category: 'other', dayNumber: lastDay, lat: 0, lon: 0, mood: null, personalNote: '' };
    ...
    try {
      const body = {
        ...
        places: steps.filter(s => s.name.trim()).map((s, i) => ({
          ...
          category: s.category || 'other', orderIndex: i, dayNumber: s.dayNumber,
    ...
    <View style={ls.root}>
      ...
      >
        ...
        <View style={ls.headerCenter}>
          <Text style={ls.headerTitle}>{phase === 'input' ? ce('heroInput') : ce('heroReview')}</Text>
        ...
        {phase === 'review' ? (
          <TouchableOpacity style={[ls.saveBtn, saving && ls.disabled]} onPress={handleSave} disabled={saving}>
            {saving ? <ActivityIndicator size="small" color="#fff" /> : <Text style={ls.saveBtnText}>{ce('done')}</Text>}
          </TouchableOpacity>
        ) : (
          <View style={{ width: 64 }} />
        )}
      ...
      {phase === 'input' ? (
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <ScrollView contentContainerStyle={[ls.scroll, { paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            ...
            <View style={ls.section}>
              ...
              <View style={ls.destBox}>
                ...
                <TextInput style={ls.destInput} value={destQuery} onChangeText={searchDestination} placeholder={ce('destPlaceholder')} placeholderTextColor="#9ca3af" autoFocus />
                {destSearching && <ActivityIndicator size="small" color={COLORS.accent} />}
              </View>
              {destResults.length > 0 && (
                <View style={ls.destResults}>
                  {destResults.map((r, i) => (
                    <TouchableOpacity key={i} style={[ls.destResult, i < destResults.length - 1 && ls.destResultBorder]} onPress={() => selectDestination(r)}>
            ...
            <View style={ls.countersRow}>
              <View style={[ls.section, { flex: 1 }]}>
                ...
                <View style={ls.stepperBox}>
                  <TouchableOpacity style={[ls.stepperBtn, days <= 1 && ls.stepperBtnOff]} onPress={() => setDays(d => Math.max(1, d - 1))} disabled={days <= 1}>
                    <Ionicons name="remove" size={22} color={days <= 1 ? '#D1D5DB' : COLORS.accent} />
                  </TouchableOpacity>
                  <View style={ls.stepperMid}><Text style={ls.stepperNum}>{days}</Text><Text style={ls.stepperUnit}>{days === 1 ? ce('day') : ce('days')}</Text></View>
                  <TouchableOpacity style={[ls.stepperBtn, days >= 30 && ls.stepperBtnOff]} onPress={() => setDays(d => Math.min(30, d + 1))} disabled={days >= 30}>
              ...
              <View style={[ls.section, { flex: 1 }]}>
                ...
                <View style={ls.stepperBox}>
                  <TouchableOpacity style={[ls.stepperBtn, travelers <= 1 && ls.stepperBtnOff]} onPress={() => setTravelers(t => Math.max(1, t - 1))} disabled={travelers <= 1}>
                    <Ionicons name="remove" size={22} color={travelers <= 1 ? '#D1D5DB' : COLORS.accent} />
                  </TouchableOpacity>
                  <View style={ls.stepperMid}><Text style={ls.stepperNum}>{travelers}</Text><Text style={ls.stepperUnit}>{travelers === 1 ? ce('person') : ce('people')}</Text></View>
                  <TouchableOpacity style={[ls.stepperBtn, travelers >= 20 && ls.stepperBtnOff]} onPress={() => setTravelers(t => Math.min(20, t + 1))} disabled={travelers >= 20}>
            ...
            <View style={ls.section}>
              ...
              <TextInput style={ls.intentionInput} value={intention} onChangeText={setIntention} placeholder={`e.g. ${destination?.name ?? '…'}`} placeholderTextColor="#b0b8c4" multiline numberOfLines={3} maxLength={400} textAlignVertical="top" />
              <Text style={ls.intentionHint}>{ce('intentionHint')}</Text>
            </View>

            {/* Generate CTA */}
            <TouchableOpacity style={[ls.genBtn, (!destination || generating) && ls.disabled]} onPress={handleGenerate} disabled={!destination || generating} activeOpacity={0.85}>
              {generating ? (
                <><ActivityIndicator size="small" color="#fff" /><Text style={ls.genBtnText}>{ce('building', { destination: destination?.name ?? '' })}</Text></>
              ) : (
                <><Ionicons name="flash-outline" size={22} color="#fff" /><Text style={ls.genBtnText}>{ce('buildExperience')}</Text></>
              )}
            </TouchableOpacity>
          </ScrollView>
        </KeyboardAvoidingView>
      ) : (
        <ScrollView contentContainerStyle={[ls.scroll, { paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
          ...
          <View style={ls.card}>
            <View style={ls.timelineTop}>
              <Text style={ls.timelineCount}>{steps.length} {ce('moments')} · {days} {days === 1 ? ce('day') : ce('days')}</Text>
            ...
            {(isMultiDay ? dayNumbers : [null]).map(day => (
              <View key={day ?? 'all'}>
                ...
                {(day !== null ? dayMap[day] : steps).map((step, idx) => {
                  ...
                  return <EditableStep key={step._key} step={step} isLast={idx === list.length - 1} onEdit={() => openEdit(step)} />;
                })}
              </View>
            ))}
      ...
      {/* Edit step modal */}
      <Modal visible={editingKey !== null} animationType="slide" transparent onRequestClose={() => { setEditingKey(null); setEditDraft(null); }}>
          ...
          <View style={ls.modalSheet}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={ls.moodRow}>
              {MOODS.map(m => {
                ...
                return (
                  <TouchableOpacity key={m.key} style={[ls.moodChip, on && ls.moodChipOn]} onPress={() => setEditDraft(d => ({ ...d, mood: d.mood === m.key ? null : m.key }))}>
            ...
            <TextInput style={[ls.editInput, ls.editTextarea, ls.noteInput]} value={editDraft?.personalNote ?? ''} onChangeText={v => setEditDraft(d => ({ ...d, personalNote: v }))} placeholder={ce('whyPlaceHint')} placeholderTextColor="#b0b8c4" multiline maxLength={300} />
  ...
  return (
    <TouchableOpacity style={etl.row} onPress={onEdit} activeOpacity={0.72}>
      ...
      <View style={[etl.content, isLast && etl.contentLast]}>
        ...
        <Text style={etl.name} numberOfLines={1}>{step.name || 'Tap to add name…'}</Text>
...
const etl = StyleSheet.create({
  ...
  moodTag: { flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: '#E8F4F2', borderRadius: 999, paddingVertical: 2, paddingHorizontal: 7 },
...
const ls = StyleSheet.create({
  ...
  saveBtn: { backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 999, paddingVertical: 7, paddingHorizontal: 16, borderWidth: 1, borderColor: 'rgba(255,255,255,0.35)' },
  ...
  destBox: { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: '#fff', borderRadius: 14, paddingHorizontal: 14, paddingVertical: 13, borderWidth: 1.5, borderColor: '#E5E7EB', ...shadow(2, 0.06, 6, 2) },
  destInput: { flex: 1, fontSize: 15, color: '#111827' },
  destResults: { backgroundColor: '#fff', borderRadius: 12, marginTop: 4, borderWidth: 1, borderColor: '#E5E7EB', ...shadow(2, 0.06, 6, 2) },
  ...
  stepperBox: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#fff', borderRadius: 14, padding: 10, borderWidth: 1, borderColor: '#E5E7EB' },
  stepperBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#F3F4F6', borderWidth: 1, borderColor: '#E5E7EB', alignItems: 'center', justifyContent: 'center' },
  ...
  catCard: { width: '30.5%', paddingVertical: 10, paddingHorizontal: 6, borderRadius: 12, borderWidth: 1.5, borderColor: '#E5E7EB', backgroundColor: '#fff', alignItems: 'center', gap: 3 },
  ...
  intentionInput: { backgroundColor: '#fff', borderRadius: 14, padding: 13, borderWidth: 1.5, borderColor: '#E5E7EB', fontSize: 14, color: '#111827', lineHeight: 21, minHeight: 80, fontStyle: 'italic' },
  intentionHint: { fontSize: 11, color: '#9ca3af' },
  genBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, backgroundColor: COLORS.primary, borderRadius: 16, paddingVertical: 18, marginTop: 8, ...shadow(4, 0.2, 12, 4) },
  genBtnText: { color: '#fff', fontSize: 17, fontWeight: '800' },
  card: { backgroundColor: '#fff', borderRadius: 14, padding: 16, borderWidth: 1, borderColor: '#E5E7EB', ...shadow(1, 0.04, 4, 1) },
  ...
  regenBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingVertical: 4, paddingHorizontal: 10, borderRadius: 999, borderWidth: 1, borderColor: COLORS.primary + '50', backgroundColor: COLORS.bgLight },
  ...
  addStepBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 12, paddingVertical: 10, borderWidth: 1.5, borderStyle: 'dashed', borderColor: COLORS.primary + '60', borderRadius: 10, justifyContent: 'center', backgroundColor: COLORS.bgLight },
  ...
  modalSectionLabel: { fontSize: 10, fontWeight: '700', color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8, marginTop: 4 },
  editTypeRow: { gap: 6, paddingBottom: 12 },
  typeChip: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingVertical: 6, paddingHorizontal: 11, borderRadius: 999, borderWidth: 1.5, borderColor: '#E5E7EB', backgroundColor: '#F9FAFB' },
  typeChipLabel: { fontSize: 12, color: '#6b7280', fontWeight: '500' },
  editInput: { borderWidth: 1.5, borderColor: '#DDE3EC', borderRadius: 10, backgroundColor: '#F7F9FC', paddingVertical: 11, paddingHorizontal: 13, fontSize: 15, color: '#111827', marginBottom: 10 },
  editTextarea: { minHeight: 72, textAlignVertical: 'top' },
  moodRow: { gap: 6, paddingBottom: 10 },
  moodChip: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingVertical: 6, paddingHorizontal: 12, borderRadius: 999, borderWidth: 1.5, borderColor: '#E5E7EB', backgroundColor: '#F9FAFB' },
  ...
  noteInput: { backgroundColor: '#FDFCF9', borderColor: '#E9E4D8', fontStyle: 'italic' },
  modalActions: { flexDirection: 'row', gap: 10, marginTop: 4 },
  deleteBtn: { flex: 1, paddingVertical: 13, borderRadius: 12, alignItems: 'center', backgroundColor: '#FEF2F2', borderWidth: 1, borderColor: '#FECACA' },
